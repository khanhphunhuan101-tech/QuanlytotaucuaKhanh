...
import { Calendar, TrainFront, Save, Clock, Trash2, Share2, Edit2, X, FileText, Image as ImageIcon, Download, Eye } from 'lucide-react';
...
export const AssignmentView: React.FC<AssignmentViewProps> = ({ onAddNotification, onImageClick }) => {
    const [trainId, setTrainId] = useState('SE1');
    ...
    useEffect(() => {
        try {
            const saved = localStorage.getItem('assignment_history');
            if (saved) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!trainId || (!content && files.length === 0)) {
        ...
        try {
            if (editingId) {
                const updatedHistory = history.map(item => 
                    item.id === editingId 
                    ? { ...item, trainId, date, content, files, timestamp: new Date().toLocaleString('vi-VN') + ' (Đã sửa)' } 
                    : item
                );
                localStorage.setItem('assignment_history', JSON.stringify(updatedHistory));
                setHistory(updatedHistory);
                
                onAddNotification('Cập nhật phân công', `Đã sửa phân công tàu ${trainId}`, { 
                    content: `Tàu: ${trainId}\nNgày: ${date}\nNội dung: ${content}`, 
                    files: files 
                });
                ...
                onAddNotification('Phân công mới', `Đã lưu phân công tàu ${trainId}`, { 
                    content: `Tàu: ${trainId}\nNgày: ${date}\nNội dung: ${content}`, 
                    files: files 
                });
                ...
                setContent(''); 
                setFiles([]);
                showToast('Đã lưu phân công và gửi thông báo!');
            }
        } catch (error) {
    ...
    const handleEdit = (record: AssignmentRecord) => {
        ...
        setTrainId(record.trainId);
        setDate(record.date);
        setContent(record.content);
        setFiles(record.files || []);
        window.scrollTo({ top: 0, behavior: 'smooth' });
        showToast('Đang sửa phân công...');
    };
    ...
    const handleShare = async (record: AssignmentRecord) => {
        const savedCrew = localStorage.getItem('crew_list');
        const crewList: any[] = savedCrew ? JSON.parse(savedCrew) : [];
        
        let shareText = `🚂 PHÂN CÔNG TÀU ${record.trainId}\n`;
        shareText += `📅 Ngày: ${record.date}\n`;
        shareText += `--------------------------------\n`;
        shareText += `📝 NỘI DUNG:\n${record.content || '(Xem tệp đính kèm)'}\n\n`;
        
        if (crewList.length > 0) {
            shareText += `👥 DANH SÁCH TỔ TÀU:\n`;
            crewList.forEach((m, i) => {
                 shareText += `${i + 1}. ${m.name} (${m.role}) - ${m.phone}\n`;
            });
        }
        ...
        if (navigator.share) {
            try {
                const shareData: ShareData = {
                    title: `Phân công tàu ${record.trainId}`,
                    text: shareText
                };
        ...
        alert('Đã sao chép nội dung phân công! Bạn có thể dán vào tin nhắn.');
    };
        ...
        <div className="space-y-6 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-gray-900">Phân Công</h2>
                        <p className="text-sm text-gray-500">Quản lý lịch trình chạy tàu</p>
                    </div>
                </div>
                {editingId && (
            ...
            <form onSubmit={handleSubmit} className={`space-y-4 transition-all duration-300 ${editingId ? 'ring-2 ring-blue-400 rounded-2xl p-1' : ''}`}>
                <div className="bg-gray-50 p-5 rounded-2xl border border-gray-100 space-y-4">
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1.5 ml-1">Mác tàu</label>
                        <div className="relative group">
                            <TrainFront className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-blue-500 group-focus-within:text-blue-600 transition-colors" />
                            <input
                                type="text"
                                value={trainId}
                                onChange={(e) => setTrainId(e.target.value)}
                                className="w-full pl-12 pr-4 py-4 rounded-xl bg-white border border-gray-200 shadow-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all font-bold text-gray-800"
                                placeholder="VD: SE1"
                            />
                        </div>
                    </div>
                    <div>
                        ...
                        <div className="relative group">
                            <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-blue-500 group-focus-within:text-blue-600 transition-colors" />
                            <input
                                type="date"
                                value={date}
                                onChange={(e) => setDate(e.target.value)}
                                className="w-full pl-12 pr-4 py-4 rounded-xl bg-white border border-gray-200 shadow-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all font-medium text-gray-800"
                            />
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1.5 ml-1">Nội dung phân công</label>
                        <textarea
                            value={content}
                            onChange={(e) => setContent(e.target.value)}
                            rows={5}
                            className="w-full p-4 rounded-xl bg-white border border-gray-200 shadow-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all text-gray-800 placeholder:text-gray-400"
                            placeholder="Nhập chi tiết nhiệm vụ..."
                        ></textarea>
                    </div>
                ...
                <button type="submit" className={`w-full text-white font-bold py-4 rounded-xl shadow-lg transition-all flex items-center justify-center space-x-2 ${editingId ? 'bg-orange-500 hover:bg-orange-600 shadow-orange-200' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-200 active:scale-95'}`}>
                    {editingId ? <Edit2 className="w-5 h-5" /> : <Save className="w-5 h-5" />}
                    <span>{editingId ? 'Cập Nhật Thay Đổi' : 'Lưu Phân Công'}</span>
                </button>
            </form>
            ...
            {history.length > 0 && (
                <div className="mt-8 animate-in slide-in-from-bottom-8 duration-500">
                    ...
                    <div className="space-y-4">
                        {history.map((record) => (
                            <div key={record.id} className={`bg-white p-4 rounded-2xl border shadow-sm hover:shadow-md transition-all group relative ${editingId === record.id ? 'border-blue-500 ring-1 ring-blue-500' : 'border-gray-100'}`}>
                                <div className="flex justify-between items-start mb-2 border-b border-gray-50 pb-2">
                                    <div className="flex items-center space-x-2">
                                        <span className="text-sm font-bold text-white bg-blue-600 px-2.5 py-0.5 rounded-lg shadow-sm shadow-blue-200">{record.trainId}</span>
                                ...
                                <div className="pt-1">
                                    <p className="text-sm text-gray-700 whitespace-pre-line leading-relaxed">{record.content}</p>
                                    ...
                                    <p className="text-[10px] text-gray-400 mt-2 text-right italic">{record.timestamp}</p>
                                </div>
                            </div>
                        ))}
                    </div>
